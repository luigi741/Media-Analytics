...
const reddit_username = '_I93bdssvL7q7Q';
const reddit_password = 'Pa6RiGU7v2-WHK2efGYiVSNkkY0';
...
//This is used to create reddit, function primarily used during testing phase. 
app.get('/createreddit', (req, res) => {
	console.log('GET /createreddit');
	var pQuery = 'CREATE TABLE reddit( );';
	...
	pool.query(pQuery, (err, result) => {
		if (err) {
			...
			console.log('reddit table has been created');
			res.send('reddit table has been created');
		}
	}); 
});
...
app.get('/reddit', (req, res) => {
	...
	const redditAPI = `https://www.reddit.com/r/${req.query.subreddit}/search.json?q=${req.query.keyword}&restrict_sr=on&include_over_18=on&sort=relevance&t=all`; // `https://api.twitter.com/1.1/search/tweets.json?q=3%23${req.query.keyword}%20-filter%3Aretweets&result_type=recent&tweet_mode=extended&lang=en`;
	var options = {
		url: redditAPI,
		headers: {
	...
	function requestCallback(error, response, body) {
		if (!error && response.statusCode == 200) {
			...
			res.send('Reddit API request successful!');
		}
		else {
			console.log('Reddit API unsuccessful', response);
			res.send('There was an error making a request to the Reddit API');
		}
	}
